...
($ => {
	//测试:10-adv-event.ts
	$.event.special.payoff = {		// name: 用于trigger

		noBubble: false, // 默认false,实测为true并不会阻止冒泡捕获 
		 ...
		 */
		setup: function (this, data, namespace, eventHandle) {
		 ...
		 */
		trigger: function (this, event, data) {
		 ...
		 */
		preDispatch: function (this, event) {
		 ...
		 * @param data 事件触发端trigger时携带的数据,不是监听端调用on携带的data
		 */
		handle: function (this, event, data) {
		 ...
		 * @param event 事件对象,包含但不限于data,以及本身已经包含data的handleObject 
		 */
		postDispatch: function (this, event) {
		 ...
		 * 			其他: 正常调用
		 */
		_default: function (this, event, data) {
		 ...
		 */
		remove: function (this, handleObj) {
	...
	$.event.special.throttledScroll = {
		 ...
		 *  true 或者不返回(即返回void): 不作如上保存,只能继续采用on监听
		 */
		setup: function (this: HTMLElement, data, namespace, eventHandle) {
			let timer = 0;
	...
	$.event.special.mousemovehalt = {
		 ...
		 * 
		 */
		setup: function (this: EventTarget, options: MousemovehaltClaim, namespace: string) {

			const opt = $.extend(true, {}, {
				delay: 300,						// 默认停顿声明
			} as MousemovehaltClaim, options);

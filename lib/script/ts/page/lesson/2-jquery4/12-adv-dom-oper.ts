...
JscssLoader.getInstance().startEntry({
	...
	main: () => {
		...
		//但也增加了其耦合度
		const SortKeyMethods: Guoshi.SortKeyMethodContainer = {
			/**
		...
		function configTableThreeSortable() {
			...
			$.getJSON('/lib/script/json/12-adv-dom-oper/books.json', (data: Row[]) => {
		...
		function createElementTersely() {
			$('table').each(function (idx: number) {
				const jtable = $(this);
				$('<h3></h3>', {
					...
					css: {
						glowColor: '#00ff00', //该属性采用后续的css挂钩注入
						color: '#ff0000',
					}
				}).insertBefore(jtable);
			});

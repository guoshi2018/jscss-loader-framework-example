...
JscssLoader.getInstance().startEntry({
	...
	main: () => {
		...
		function configReadmore() {
			...
			$('a.more').click((evt: JQuery.ClickEvent) => {
				evt.preventDefault();
		...
		function configMockButtons() {
			...
			$('#mock-slideToggle').click((evt: JQuery.ClickEvent) => {
				const jp1 = $('p').eq(1);
				jp1.animate({
					height: 'toggle',
					margin: 'toggle',
			...
			$('#anim-switcher-box').click((evt: JQuery.ClickEvent) => {
				const pw = $('div.speech p').outerWidth() as number;
				const jsw = $('div#switcher');
				const sww = jsw.outerWidth() as number;
			...
			$('#anim-queue-switcher-box').click((evt: JQuery.ClickEvent) => {
				const pw = $('div.speech p').outerWidth() as number;
				const jsw = $('div#switcher');
				const sww = jsw.outerWidth() as number;
			...
			$('#anim-opacity-move-at-the-same-time').click((evt: JQuery.ClickEvent) => {
				const pw = $('div.speech p').outerWidth() as number;
				const jsw = $('div#switcher');
				const sww = jsw.outerWidth() as number;
				jsw.css('position', 'relative')
					...
					.animate({
						...
						duration: 5000, //至完全透明与高度增大同时进行
						queue: false,
					})
					.slideUp('slow')
					.queue((next: () => void) => {//非效果方法添加到队列的方法 1/2
			...
			$('#mult-ele-anim').click((evt: JQuery.ClickEvent) => {
				$('p').eq(2).css('border', '1px solid #333')
					.one('click', (evt: JQuery.ClickEvent) => { //点击,则两段一上一下
			...
			$('#mult-ele-anim-step').click((evt: JQuery.ClickEvent) => {
				const jclickedItem = $('p').eq(2).css('border', '1px solid #333')
					.one('click', function (evt: JQuery.ClickEvent) { //点击,则两段一上一下
			...
			$('#mult-ele-by-queue').click((evt: JQuery.ClickEvent) => {
				$('p').eq(2).css('border', '1px solid #333')
					.one('click', function (evt: JQuery.ClickEvent) {
						$(evt.target).next().slideDown(3000)
							.queue(() => {

...
JscssLoader.getInstance().startEntry({
  ...
  main: () => {
    ...
    function configMockButtons() {
      ...
      $('#use-attr').click(() => {
        $('div.chapter a[href*=wikipedia]').attr({ //same as href*="wikipedia"
          //attr方法,不论何种属性, 均出现在html中
          rel: 'external',
          title: function (index: number, oldTitle: string) {
            return `Learn more about ${$(this).text()} at Wikipedia`;
          },
          id: (index: number, oldId: string) => `wikilink-${index}`,
          three: 'three-value',
          four: 'four-value',
        });
      });
      ...
      $('#use-prop').click(() => {
        $('div.chapter a[href*=wikipedia]').prop({
          //非固有属性,不会出现在html中
          one: 'one-value',
          two: function (idx: number, oldV: string) {
      ...
      $('#transfer-footnote1').one('click', (evt: JQuery.ClickEvent) => {
        //简单的提取脚注到footer前
      ...
      //这是书上原版P94
      $('#transfer-footnote2').one('click', (evt: JQuery.ClickEvent) => {
        //简单的提取脚注到footer前
        //$('span.footnote').insertBefore('div#footer');
      ...
      //这是书上原版P95
      $('#transfer-footnote3').one('click', (evt: JQuery.ClickEvent) => {
        const jnotes = $('<ol id="notes"></ol>').insertBefore('#footer');
        $('span.footnote').each((idx: number, ele: HTMLElement) => {
      ...
      //书上原版P96
      $('#transfer-footnote4').one('click', (evt: JQuery.ClickEvent) => {
        const jnotes = $('<ol id="notes"></ol>').insertBefore('#footer');
        $('span.footnote').each((idx: number, ele: HTMLElement) => {
          const n = idx + 1;
          $(ele)
      ...
      $('#clone-elements-without-event').one('click', (evt: JQuery.ClickEvent) => {

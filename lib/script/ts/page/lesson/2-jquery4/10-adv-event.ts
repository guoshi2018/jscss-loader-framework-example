...
JscssLoader.getInstance().startEntry({
	...
	main: () => {
		...
		function configLoadmoreByHyperlink() {
			 ...
			 * 内容可见
			 */
			$('a#more-photos').click(function (evt, scrollToVisible?: boolean) {
				evt.preventDefault();
				nextPage++;
		...
		function configDocumentCustomEvent() {
			...
			$(document).on('loadmore', function (_, scrollToVisible?: boolean) {
		...
		function extensionEvenAccordingToBook() {
			...
			$(document).on('nextPage', function (_, scrollToVisible: boolean) {
		...
		function testScrollBarMethod() {
			...
			async function examScroll(evt: JQuery.ClickEvent) {
		...
		function testExtEvent2() {
			...
			$('input.start').on('click', function () {
				function hualongHandler(evt: JQuery.TriggeredEvent, data: any) {
					console.log('[jouter on payoff.hualong.wenshan.yunnan self ] middle receive: ',
						evt, data);
		...
		function chapterExercise() {
			...
			//2
			$(window).on('pageNext', (evt: JQuery.Event, pageIndex: number) => {
			...
			//4. 5.
			(() => {
				function printMouseLocation(ele: HTMLElement, evt: JQuery.MouseMoveEvent) {

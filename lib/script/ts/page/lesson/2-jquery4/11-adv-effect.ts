...
JscssLoader.getInstance().startEntry({
	...
	main: () => {
		...
		function configHoverImage() {
			 ...
			 */
			function correctParams(eventType: string) {
				let size = 85;
				let padding = 0;
			 ...
			 */
			function switchByEvent(jele: JQuery, evt: JQuery.TriggeredEvent) {
			...
			$('div.member').on('mouseenter mouseleave', function (evt) { //需要this
				const jimg = $('img', this);
		 ...
		 */
		function configAlertClosingPage(idleBysecond: number, warnBysecond: number) {
			const delay = idleBysecond * 1000;
			$(window).on('mousemovehalt', {
				delay,	//2秒钟鼠标不动
			}, async () => {
				console.log(`------------------------------------------`);
				console.log(`The mouse has stopped active for ${idleBysecond
					} second(s),` +
		...
		function configClickPhotoToToShowDetailsThenBio() {
			...
			const bioBaseStyle = {	//简介基础style
				display: 'none', //或者 opacity: 0, 
				height: '5px', //0可能更好
				width: '25px',
			};
			const bioEffects = {	//动画效果控制
				duration: 800,
				easing: 'easeOutQuart',
				specialEasing: {
					opacity: 'linear',
			 ...
			 */
			function showBio(this: HTMLElement) {
				...
				jmovable
					...
					.queue(next => {
						jmovable.css(startStyle as JQuery.PlainObject)
						next();
					})
					.animate(endStyle, bioEffects)
					.animate({

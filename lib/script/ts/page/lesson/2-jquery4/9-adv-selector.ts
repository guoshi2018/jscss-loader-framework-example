...
JscssLoader.getInstance().startEntry({
	...
	main: () => {
		...
		function configHyperlink() {
			const jlink = $('#topics a').click(function (evt) {
				...
				if (topic != 'All') {		//3
					...
					$('#news tr td:nth-child(4)').filter(function () {	//ok
						return $(this).text() != topic;
					}).parent('tr').hide();
		...
		function showJqueryObjectProperties() {
			...
			function threeProp(jq: JQuery<HTMLElement>, name: string) {
		 ...
		 */
		function testCustomGroupSelector(num: number) {
		...
		function testDomElementStack() {
			const jrel = $('#release');
			console.log('是否相等:',
				$('#release').nextAll().prevObject === jrel,		// false,因为源头是重塑的
			...
			console.log(
				jnext.prevObject === jrel.nextAll().end(),	//false
				jnext.prevObject === jnext.end(),			//true
				jnext.addBack().prevObject === jnext,		//true
			);

...
JscssLoader.getInstance().startEntry({
	...
	main: () => {
		...
		function enable_disable_anim_demo(): void {
		...
		function fade_demo() {
			...
			function showProps(this: HTMLElement) {
				let jele = $(this);
				console.log(jele.width(), jele.height(), jele.css('opacity'),
					jele.css('display'));
			}
		...
		function queue_demo() {
			$('.queue_demo').click(function () {
				$(this).animate({
					...
					.queue((nextFunction: (s: string) => void) => {      //Another way of dequeuing is by calling the function
						$(this).text('宽度变换完毕');
						nextFunction('hello,world'); // ok,
					})
					.queue(function (arg) { //注意arg不代表前面的hello world
